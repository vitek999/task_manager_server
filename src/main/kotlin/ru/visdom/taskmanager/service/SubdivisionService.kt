...
class SubdivisionService(private val subdivisionRepository: SubdivisionRepository) {
    fun getAll(): Iterable<Subdivision> = subdivisionRepository.findAllNative()

    fun getById(id: Long): Subdivision? = subdivisionRepository.findByIdNative(id)
    ...
    fun update(subdivision: Subdivision) = subdivisionRepository.updateNative(subdivision.id, subdivision.name, subdivision.description, subdivision.companyId)
}